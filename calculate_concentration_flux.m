function [ concentration_flux ] = calculate_concentration_flux( Pvector, ...
    Cvector, h, Lz, Q0, c0, km, mu, D, xbounds, islow, direction, ...
    meshx, meshy, meshz )
          ...
          if islow
              if onBoundary([true false false],xbounds,node_coord_matrix(el_nodes,:))
                  ...
                  concentration_flux(elz) = concentration_flux(elz) - ...
                      direction*yspacing*Lz/(Q0*c0)*(D*dcdx + km/mu*dpdx*center_c);
              end
          else
              if onBoundary([true false false],xbounds,node_coord_matrix(el_nodes,:))
                  dxdxi = sum(node_coord_matrix(el_nodes,1).*dphisdxis);
...
concentration_flux = concentration_flux./(NYNodes-1);
end

...
function [Integral,bxlow,bxhigh,bylow,byhigh,bzlow,bzhigh,non_zero_lowy,...
    byhighcheck] = calculate_pressure_flux(Pvector, Cvector, kbm, km, mu,...
    bm_thick, x,y,z, xbounds,ybounds,zbounds)
% Create gauss points
gp = 1/sqrt(3);
...
lowxsurface_gauss_points(1,:) = [-1 -gp -gp 1];
lowxsurface_gauss_points(2,:) = [-1 gp -gp 1];
lowxsurface_gauss_points(3,:) = [-1 -gp gp 1];
lowxsurface_gauss_points(4,:) = [-1 gp gp 1];
...
highxsurface_gauss_points(1,:) = [1 -gp -gp 1];
highxsurface_gauss_points(2,:) = [1 gp -gp 1];
highxsurface_gauss_points(3,:) = [1 -gp gp 1];
highxsurface_gauss_points(4,:) = [1 gp gp 1];
...
lowysurface_gauss_points(1,:) = [-gp -1 -gp 1];
lowysurface_gauss_points(2,:) = [-gp -1 gp 1];
lowysurface_gauss_points(3,:) = [gp -1 -gp 1];
lowysurface_gauss_points(4,:) = [gp -1 gp 1];
...
highysurface_gauss_points(1,:) = [-gp 1 -gp 1];
highysurface_gauss_points(2,:) = [-gp 1 gp 1];
highysurface_gauss_points(3,:) = [gp 1 -gp 1];
highysurface_gauss_points(4,:) = [gp 1 gp 1];
...
phis = get_phis(highysurface_gauss_points, 8);

lowz_gauss_points(1,:) = [-gp -gp -1 1];
lowz_gauss_points(2,:) = [-gp gp -1 1];
lowz_gauss_points(3,:) = [gp -gp -1 1];
lowz_gauss_points(4,:) = [gp gp -1 1];
...
highz_gauss_points(1,:) = [-gp -gp 1 1];
highz_gauss_points(2,:) = [-gp gp 1 1];
highz_gauss_points(3,:) = [gp -gp 1 1];
highz_gauss_points(4,:) = [gp gp 1 1];
...
%                         real_dphidetas = dphidetas(lowy_phis,gp);
                        ...
                        dphi = zeros(8,3);
                        surface_pressure = sum(Pvector(el_nodes).*phis(:,gp));
                        ...
                        Jacobian = [dxdxi_surf dxdzeta_surf; ...
                         ...
                         J = det(Jacobian);
                            ...
                            byhigh = byhigh - ...
                                km/mu*abs(J)*dpdy;
                            % Changed to km, not kbm, because this seems to
                            % be what we need
                            for phi=1:8
                                byhighcheck = byhighcheck - ...
                                    kbm/(mu*bm_thick)*(surface_pressure -...
                                    (-33.7+1.5*Cvector(el_nodes(phi))+0.2*Cvector(el_nodes(phi))^2)...
                                    *133.3)*abs(J)*phis(phi,gp);
                            end
...
%                         real_dphidzetas = dphidzetas(highz_phis,gp);
                        ...
                        J = det(Jacobian);
                            ...
                            bzhigh = bzhigh - km/mu*abs(J)*dpdz;
